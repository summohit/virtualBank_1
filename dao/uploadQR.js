const uploadQRModel = require("../models/uploadQRCode");

const insert = async (data) => {
  try {
    const uploadQrData = new uploadQRModel(data);
...
const getAll = async (query) => {
  try {
    ...
    let result = await uploadQRModel.aggregate(query);
...
const getById = async (qrCodeId) => {
  try {
    let result = await uploadQRModel.findOne({ _id: qrCodeId });
    console.log("result", result);
    return result;
  } catch (error) {
    console.log(error.stack);
...
const updateById = async (qrCodeId, payload) => {
  try {
    let result = await uploadQRModel.updateOne(
      { _id: qrCodeId },
      { $set: { ...payload } }
    );
    return result;
